  ...
  return (
    <StyleItem onClick={() => onClick(id)}>
      ...
      >
        ...
        >
          <CrewP>{name}</CrewP>
          <CrewP style={{ fontSize: '0.6vw', color: colors.crewGray2 }}>
            {introduce}
          </CrewP>
        </div>
      </div>
      ...
      <div
        className="crewBottomBox"
        style={{ display: 'flex', marginTop: '1vw', gap: '0.7vw' }}
      >
        <BottomBox style={{ width: '4.1vw' }}>
          Point<CrewP2>{points}</CrewP2>
        </BottomBox>
        <BottomBox style={{ width: '6.75vw' }}>
          헌혈참여율<CrewP2>{participationRate}</CrewP2>%
        </BottomBox>
        <BottomBox style={{ width: '4.75vw' }}>
          인원<CrewP2>{memberCount}</CrewP2>명
        </BottomBox>
      </div>
    </StyleItem>
  );
